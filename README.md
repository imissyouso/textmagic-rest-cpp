...
```shell
wget https://github.com/imissyouso/textmagic-rest-cpp/archive/v2.0.476.tar.gz && \
tar zxf v2.0.476.tar.gz && \
rm -f v2.0.476.tar.gz && \
cd textmagic-rest-cpp-2.0.476
```
Build using cmake
```shell
cmake . && cmake --build .
```
The output library file will be placed in `textmagic-rest-cpp-2.0.476/lib` directory:

## Usage Example
In the example below, we assume that you moved the library sources `textmagic-rest-cpp-2.0.476` directory to your test project root directory.
...
set_property(TARGET textmagic_client PROPERTY IMPORTED_LOCATION "${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-2.0.476/lib/libtextmagic_client.so")

target_include_directories (app PRIVATE ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-2.0.476 ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-2.0.476/model ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-2.0.476/api)
...
#include <iostream>
#include <fstream>
#include "textmagic-rest-cpp-2.0.476/ApiClient.h"
#include "textmagic-rest-cpp-2.0.476/ApiConfiguration.h"
#include "textmagic-rest-cpp-2.0.476/api/TextMagicApi.h"
