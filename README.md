...
```shell
wget https://github.com/imissyouso/textmagic-rest-cpp/archive/v2.0.296.tar.gz && \
tar zxf v2.0.296.tar.gz && \
rm -f v2.0.296.tar.gz && \
cd textmagic-rest-cpp-2.0.296
```
Build using cmake
```shell
cmake . && cmake --build .
```
Output library file will be placed to `textmagic-rest-cpp-2.0.296/lib` directory

## Usage Example
In example below we assume that you moved library sources `textmagic-rest-cpp-2.0.296` directory to your test project root directory.
...
set_property(TARGET textmagic_client PROPERTY IMPORTED_LOCATION "${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-2.0.296/lib/libtextmagic_client.so")

target_include_directories (app PRIVATE ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-2.0.296 ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-2.0.296/model ${PROJECT_SOURCE_DIR}/textmagic-rest-cpp-2.0.296/api)
...
#include <iostream>
#include <fstream>
#include "textmagic-rest-cpp-2.0.296/ApiClient.h"
#include "textmagic-rest-cpp-2.0.296/ApiConfiguration.h"
#include "textmagic-rest-cpp-2.0.296/api/TextMagicApi.h"
